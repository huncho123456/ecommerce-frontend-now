...
export let guestId = null;  // ⬅️ this makes it exportable
...
async function getCartByGuestId(guestId) {
  try {
    ...
    const response = await fetch(`${config.API_BASE_URL}/carts/${guestId}`, {
      method: "GET",
    ...
    const data = await response.json();
    console.log("🛍️ Fetched cart by guestId:", data);

    return data;

  } catch (err) {
    console.error("❌ Error fetching cart by guestId:", err);
  }
}
